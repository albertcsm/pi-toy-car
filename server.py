...
def action_move():
    ...
    if instruction['steering'] < 0:
        speed1 -= MAX_SPEED * 2 * abs(instruction['steering'])
    else:
        speed0 -= MAX_SPEED * 2 * abs(instruction['steering'])

    basicSpeed = MAX_SPEED * abs(instruction['forward']) - (speed0 + speed1) / 2
    ...
    if speed0 >= 0:
        Motor.MotorRun(0, 'forward', speed0)
    else:
        Motor.MotorRun(0, 'backward', -speed0)
    if speed1 >= 0:
        Motor.MotorRun(1, 'forward', speed1)
    else:
        Motor.MotorRun(1, 'backward', -speed1)
    return "move"
